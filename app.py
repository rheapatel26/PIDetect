...
def generate(chat_input):
    ...
    context_prompt = context_prompt = """Objective:
...
If any out-of-topic or out-of-domain questions are asked, politely redirect users to industry-based information and relevant engineering queries. Complete your answer and do not exceed 500 words and give responses in bullets or easy paragraphs, dont mess. 
...
def generate(chat_input):
    ...
    generate_content_config = types.GenerateContentConfig(
        temperature=1, top_p=0.95, top_k=40, max_output_tokens=200, response_mime_type="text/plain",
    )
...
# --------------- GLOBAL VARIABLES -------------------------
model2 = YOLO("finaltrain_best.pt")
class_name_mapping = {"0": "Ball Valve 1", "1": "Ball Valve 2", "2": "Ball Valve 3", "3": "Onsheet Connector", "4": "Centrifugal Fan", "5": "IHTL", "6": "Pneumatic Signal", "7": "NP"}
class_color_mapping = {"0": (255, 0, 0), "1": (0, 255, 0), "2": (0, 0, 255), "3": (255, 255, 0), "4": (255, 0, 255), "5": (0, 255, 255), "6": (128, 0, 128), "7": (0, 128, 128)}
detected_symbols = []
selector_img_wide_screen = ["assets/PIDs/2.jpeg", "assets/PIDs/3.jpeg", "assets/PIDs/5.png", "assets/PIDs/6.png"]
...
# Sidebar Filters
st.sidebar.title("Filter Classes")
selected_classes = {class_id: st.sidebar.checkbox(class_name, value=True) for class_id, class_name in class_name_mapping.items()}
...
image = Image.open(uploaded_file) if uploaded_file else Image.open(img_path) if img_path else None
if image:
    st.image(image, caption="Selected Image", use_container_width=True)
    if st.button("Predict"):
        image_np, results = img_proc(image)
